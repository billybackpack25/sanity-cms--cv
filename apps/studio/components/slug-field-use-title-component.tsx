...
/**
 * Slug field component for editing the pathname of a document.
 * If the slug field is read-only, it will automatically update the slug based on the document's title field
 */
export function PathnameFieldUseTitleComponent(
  ...
  const parent = useFormValue(props.path.slice(0, -1)) as SanityDocument;
  ...
  // If readonly, auto-update slug when title changes (if title exists)
  useEffect(() => {
    if (readOnly && parent?.title) {
      const newSlug = slugify(parent.title as string, {
        lower: true,
        remove: /[^a-zA-Z0-9 ]/g,
      });
  ...
  }, [readOnly, parent?.title, value?.current, onChange]);
